...
description: "Aprende cómo usar la Arquitectura Orientada a Eventos (EDA) con Azure Functions como consumidor, aprovechando el stack serverless para mejorar el rendimiento y los costes en un sistema de reservas hoteleras."
...
tags:
  ...
  - Serverless
img: ""
img_alt: "Gráfico de Arquitectura Orientada a Eventos"
---
...
sequenceDiagram
    ...
    participant RoomManagementService as "Servicio de Gestión de Habitaciones"
    participant BillingService as "Servicio de Facturación"
    ...
    EventBus->>RoomManagementService: Enrutar evento Guest Checkout a Gestión de Habitaciones
    RoomManagementService->>HotelReservationService: Actualizar estado de habitación (ej. cambiar de "ocupada" a "vacante")
    HotelReservationService->>EventBus: Publicar evento estado de habitación actualizado

    EventBus->>BillingService: Enrutar evento estado de habitación actualizado a Facturación
    BillingService->>HotelReservationService: Enviar notificación de factura
    ...
    EventBus->>AzureFunctions: Disparar acciones de gestión de habitaciones (ej. notificar limpieza)
    AzureFunctions->>RoomManagementService: Disparar acciones de gestión de habitaciones
    AzureFunctions->>BillingService: Disparar acciones de facturación
